...
void UHTNGraph::UpdateAsset(int32 UpdateFlags)
{
	...
	static const auto GetNextNodes = [](UAIGraphNode* GraphNode, TArray<UHTNStandaloneNode*>& OutNodes)
	{
	...
	const auto GetDecorators = [&](UEdGraphNode* GraphNode, TArray<UHTNDecorator*>& OutNodes)
	{
		OutNodes.Reset();
	...
	const auto GetServices = [&](UEdGraphNode* GraphNode, TArray<UHTNService*>& OutNodes)
	{
		OutNodes.Reset();
...
void UHTNGraph::SpawnMissingGraphNodes()
{
	...
	const auto SpawnGraphNode = [&](UHTNStandaloneNode& Node) -> UHTNGraphNode*
	{
		static const auto GetGraphNodeClassFor = [](UHTNNode& Node) -> TSubclassOf<UHTNGraphNode>
		{
		...
		UHTNGraphNode* const HTNGraphNode = CastChecked<UHTNGraphNode>(CreateNode(GraphNodeClass, /*bSelectNewNode=*/true));
	...
	const auto ConnectPinToNodes = [&](UEdGraphPin* const OutputPin, TArrayView<UHTNStandaloneNode*> NextNodes)
	{
		if (!OutputPin)
		{
			return;
