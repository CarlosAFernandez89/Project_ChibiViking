...
void FWorldstateSetValueContainerDetails::CustomizeChildren(TSharedRef<class IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	StructBuilder
		...
		[
			...
			.ShowTreeView(true)
			.SelectedClass_Lambda([=]() -> const UClass*
			{
				const UObject* Object = nullptr;
				return CachedObjectPropertyHandle->GetValue(Object) == FPropertyAccess::Success ?
					Cast<UClass>(Object) : 
					nullptr;
			})
	...
	StructBuilder
		...
		[
			...
			{
				int32 Value = 0;
				return CachedIntPropertyHandle->GetValue(Value) != FPropertyAccess::Success ? ECheckBoxState::Undetermined :
					Value != 0 ? ECheckBoxState::Checked :
					ECheckBoxState::Unchecked;
			})
			.OnCheckStateChanged_Lambda([=](ECheckBoxState NewState)
			{
				CachedIntPropertyHandle->SetValue(NewState == ECheckBoxState::Checked ? 1 : 0);
			})
		];
...
void FWorldstateSetValueContainerDetails::OnKeyIDChanged()
{
	...
	{
		CachedBaseClass = ClassKeyType->BaseClass ? UNWRAP_TOBJECT_PTR(ClassKeyType->BaseClass) : UObject::StaticClass();
	...
	{
		CachedBaseClass = ObjectKeyType->BaseClass ? UNWRAP_TOBJECT_PTR(ObjectKeyType->BaseClass) : UObject::StaticClass();
...
{
	uint8 KeyID;
	if (CachedBlackboardKeyIDProperty.IsValid() && CachedBlackboardKeyIDProperty->GetValue(KeyID) == FPropertyAccess::Success)
	{
...
TSharedRef<SWidget> FWorldstateSetValueContainerDetails::OnGetEnumValueContent() const
{
	...
	for (int32 I = 0; I < CachedEnumNames.Num(); ++I)
	{
		MenuBuilder.AddMenuEntry(
			...
			TAttribute<FText>(), 
			FSlateIcon(), 
			FExecuteAction::CreateLambda([=]() { CachedIntPropertyHandle->SetValue(I); })
		);
	}
