...
namespace
{
	...
	protected:
		...
		{
			return
				GraphPinObj->bIsDiffing ? HTNColors::Pin::Diff :
...
{
	UHTNGraphNode* const HTNGraphNode = Cast<UHTNGraphNode>(GraphNode);
	return HTNGraphNode ? FEditorStyle::GetBrush(HTNGraphNode->GetIconName()) : FEditorStyle::GetBrush(TEXT("BTEditor.Graph.BTNode.Icon"));
}
...
FSlateColor SGraphNode_HTN::GetBorderBackgroundColor() const
{
	if (UHTNGraphNode* const HTNGraphNode = Cast<UHTNGraphNode>(GraphNode))
	{
		const bool bIsSubNode = HTNGraphNode->ParentNode != nullptr;
		if (bIsSubNode)
		{
...
FSlateColor SGraphNode_HTN::GetBackgroundColor() const
{
	...
	if (UHTNGraphNode* const HTNGraphNode = Cast<UHTNGraphNode>(GraphNode))
	{
		...
		else if (Cast<UHTNTask>(HTNGraphNode->NodeInstance))
		{
