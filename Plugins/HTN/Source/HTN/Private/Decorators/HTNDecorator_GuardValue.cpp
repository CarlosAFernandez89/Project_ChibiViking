...
void UHTNDecorator_GuardValue::OnEnterPlan(UHTNComponent& OwnerComp, const FHTNPlan& Plan, const FHTNPlanStepID& StepID) const
{
	if (!BlackboardKey.SelectedKeyType)
	{
		UE_VLOG_UELOG(&OwnerComp, LogHTN, Error, TEXT("Blackboard Key %s has invalid type! Decorator %s can't plan."),
			*BlackboardKey.SelectedKeyName.ToString(), *GetNodeName()
		);
		return;
	}
...
void UHTNDecorator_GuardValue::OnExecutionFinish(UHTNComponent& OwnerComp, uint8* NodeMemory, EHTNNodeResult Result)
{
	...
	{
		const TSharedPtr<FHTNPlan> CurrentPlan = OwnerComp.GetCurrentPlan();
		if (CurrentPlan.IsValid())
		{
			for (const FHTNPlanStepID& CurrentStepID : OwnerComp.GetCurrentlyExecutingStepIDs())
			{
				if (RestoreWorldstateValue(OwnerComp, *OwnerComp.GetBlackboardProxy(), *CurrentPlan, CurrentStepID))
				{
					break;
				}
			}
...
FString UHTNDecorator_GuardValue::GetStaticDescription() const
{
	...
	return FString::Printf(TEXT("%s: %s%s%s"),
		...
		*FString::Printf(TEXT("%s"), *BlackboardKey.SelectedKeyName.ToString()),
		*SetDescription,
		*RestoreDescription
	);
}

bool UHTNDecorator_GuardValue::RestoreWorldstateValue(UHTNComponent& OwnerComp, UWorldStateProxy& WorldStateProxy, const FHTNPlan& CurrentPlan, const FHTNPlanStepID& CurrentStepID) const
{
	const TSharedPtr<const FBlackboardWorldState> WorldStateBeforeEntered = CurrentPlan.GetWorldstateBeforeDecoratorPlanEnter(*this, CurrentStepID);
	...
	UE_VLOG_UELOG(OwnerComp.GetAIOwner(), LogHTN, Error, TEXT("Could not find worldstate before decorator %s at step (level %i, step %i)"),
		*GetNodeName(),
		CurrentStepID.LevelIndex, CurrentStepID.StepIndex
	);
	return false;
}
